...
from .serializers import ProductSerializer, ProductCategorySerializer, StockSerializer, ProductBatchSerializer
...
class ProductScanView(APIView):
    def post(self, request):
        logger.debug(f"[SomPOS] Scanning barcode: {request.data}")
        ...
        try:
            ...
            if product:
                ...
                if batch_serializer.is_valid():
                    ...
                    logger.info(f"[SomPOS] Created batch for {product.name}: {batch.quantity}, expires: {batch.expiration_date}")
                    return Response({
                ...
                # Продукт не найден, создаём новый
                product_data = {
                    'barcode': barcode,
                ...
                if not all([product_data['name'], product_data['category'], product_data['sale_price']]):
                    logger.warning(f"[SomPOS] Missing required fields for new product: {product_data}")
                    return Response({"error": "Name, category, and sale_price are required for new product"}, status=status.HTTP_400_BAD_REQUEST)
                
                product_serializer = ProductSerializer(data=product_data)
                if product_serializer.is_valid():
                    product = product_serializer.save()
                    logger.info(f"[SomPOS] Created product: {product.name} with barcode: {barcode}")
                    # Создаём партию
                    batch_data = {
                        'product': product.id,
                        'quantity': quantity,
            ...
            logger.error(f"[SomPOS] Error scanning barcode: {e}")
